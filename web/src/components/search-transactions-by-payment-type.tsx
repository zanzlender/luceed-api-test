...
import { GetTransactionsByPaymentType } from "../lib/luceed-api";
import { useMutation } from "@tanstack/react-query";

export default function SearchTranscantionByPaymentType() {
  ...
  const transactionsQuery = useMutation({
    mutationFn: GetTransactionsByPaymentType,
  });
  ...
  return (
    <div className="w-full flex flex-col gap-4">
      <h2 className="text-xl font-semibold">Transactions calculation by payment type</h2>
      ...
      <div className="flex flex-col gap-2 w-full max-h-[500px] overflow-y-auto" ref={parentRef}>
        {transactionsQuery.isPending && <div>Loading...</div>}
        {transactionsQuery.isError && <div>Error while loading...</div>}
        {transactionsQuery.isSuccess &&
          transactionsQuery.data.result[0].obracun_placanja.length === 0 && <div>No results</div>}
        {transactionsQuery.data?.result[0].obracun_placanja?.map((transaction) => {
